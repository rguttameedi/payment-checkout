...
import { g as translateErrorMessage, i as initWalletEvents, S as STATES, C as COUNTRY, a as CountryAndPhoneCodes, b as isValidPOBoxAddess, o as oscilarService, h as trackAddPaymentError, c as trackValidationError, j as trackAddPaymentSuccess, k as trackAddPaymentCancelled } from './p-BIbnFGdR.js';
import { E as Environment, g as getApiConfig, d as defineCustomElement$1, C as CardType, a as AddressType, A as AccountStatus } from './p-FUsAEGQG.js';
...
async function addCard(operationsToken, userScopedAccessToken, requestBody, environment = Environment.PRODUCTION) {
    const apiConfig = getApiConfig(environment);
    const url = `${apiConfig.BASE_URL}${apiConfig.RELATIVE_URLS.ADD_CARD}`;
    devLog(environment, 'Add Card Request Details:', {
        url,
        method: 'POST',
        headers: {
    ...
    try {
        ...
        devLog(environment, 'Add Card Response Status:', response.status, response.statusText);
        const responseData = await response.json();
        devLog(environment, 'Add Card Response Data:', responseData);
        ...
        devLog(environment, 'Card added successfully:', responseData);
            ...
            if (errorResponse.errors[fieldKey] && Array.isArray(errorResponse.errors[fieldKey])) {
                ...
                for (const errorMessage of fieldErrors) {
                    ...
                    if (isFieldSpecificError(fieldKey)) {
                        ...
                        devLog(environment, `Non-field error: "${errorMessage}" → "${translatedMessage}"`);
...
const AddNewPayment = /*@__PURE__*/ proxyCustomElement(class AddNewPayment extends H {
    constructor() {
        super();
        this.__registerHost();
        this.goToPaymentSelector = createEvent(this, "goToPaymentSelector");
        this.addressValidation = createEvent(this, "addressValidation");
        this.cardAdded = createEvent(this, "cardAdded");
    }
    goToPaymentSelector;
    addressValidation;
    errorBannerRef = null;
    ...
    static get style() { return addnewpaymentModuleCss; }
}, [0, "add-new-payment", {
        "operationsToken": [1, "operations-token"],
        "userScopedAccessToken": [1, "user-scoped-access-token"],
        "environment": [1],
        "availableCreditCards": [16, "available-credit-cards"],
...
function defineCustomElement() {
    ...
    const components = ["add-new-payment", "date-picker", "mfa-model-popup"];
    components.forEach(tagName => { switch (tagName) {
        case "add-new-payment":
            if (!customElements.get(tagName)) {
                customElements.define(tagName, AddNewPayment);
            }
            break;
        case "date-picker":
            if (!customElements.get(tagName)) {
...
export { AddNewPayment as A, defineCustomElement as d };
//# sourceMappingURL=p-CBiti814.js.map
