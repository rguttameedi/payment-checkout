    ...
    renderDatePicker() {
        ...
        const renderCalendarDays = () => {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                days.push(index.h("td", null, index.h("button", { type: "button", class: `date-picker-day ${isSelected ? 'selected' : ''} ${isToday ? 'today' : ''}`, onClick: (e) => this.handleDateSelect(date, e) }, day)));
            }
            const weeks = [];
            let week = [];
            days.forEach((day, index$1) => {
                week.push(day);
                if ((index$1 + 1) % 7 === 0 || index$1 === days.length - 1) {
                    while (week.length < 7) {
                        week.push(index.h("td", null));
                    }
                    weeks.push(index.h("tr", null, week));
                    week = [];
                }
            });
            return weeks;
        ...
        if (this.isYearPickerOpen) {
            ...
            return (index.h("div", { class: "date-picker-dropdown" }, index.h("div", { class: "date-picker-header" }, index.h("div", { class: "year-picker-navigation" }, index.h("button", { type: "button", class: "year-nav-button", onClick: (e) => this.handleYearRangeChange('prev', e), disabled: isPrevDisabled }, "\u2039"), index.h("button", { type: "button", class: "year-range-button", onClick: (e) => this.toggleYearPicker(e) }, startYear, " - ", Math.min(startYear + 17, 2099)), index.h("button", { type: "button", class: "year-nav-button", onClick: (e) => this.handleYearRangeChange('next', e), disabled: isNextDisabled }, "\u203A"))), index.h("div", { class: "year-picker-grid" }, years.map(year => (index.h("button", { type: "button", class: `year-picker-item ${year === currentYear ? 'selected' : ''} ${year === today.getFullYear() ? 'today' : ''}`, onClick: (e) => this.handleYearSelect(year, e) }, year))))));
        }
        return (index.h("div", { class: "date-picker-dropdown" }, index.h("div", { class: "date-picker-header" }, index.h("div", { class: "date-picker-selectors" }, index.h("select", { class: "date-picker-month-select", onInput: handleMonthChange, onFocus: handleMonthFocus, onBlur: handleMonthBlur }, monthNames.map((month, index$1) => (index.h("option", { value: index$1, selected: index$1 === currentMonth }, month)))), index.h("button", { type: "button", class: "date-picker-year-button", onClick: (e) => this.toggleYearPicker(e) }, currentYear))), index.h("div", { class: "date-picker-calendar-wrapper" }, index.h("table", { class: "date-picker-calendar" }, index.h("thead", null, index.h("tr", null, index.h("th", null, "S"), index.h("th", null, "M"), index.h("th", null, "T"), index.h("th", null, "W"), index.h("th", null, "T"), index.h("th", null, "F"), index.h("th", null, "S"))), index.h("tbody", null, renderCalendarDays())))));
    }
    render() {
        return (index.h("div", { key: 'eae29dc3ac91b8f6399c5be49a2bd2cccead5ae0', class: "date-picker-wrapper" }, index.h("input", { key: 'cb4db79c5a172451151d4fe56b2fb102de5946f2', type: "text", id: this.inputId, placeholder: this.placeholder, value: this.inputValue, onInput: (e) => this.handleManualInput(e), maxLength: 10, autocomplete: "off" }), index.h("button", { key: '39d724ff5e9ecd6508cde1359edb63de5efb31f5', type: "button", class: "date-picker-icon", onClick: (e) => this.toggleDatePicker(e) }, index.h("svg", { key: '7cda325431da9434a3fb9b4e514279265ea8412f', xmlns: "http://www.w3.org/2000/svg", width: "20", height: "20", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": "2" }, index.h("rect", { key: '86e1b35b89b44ab94eab446ae1cc31402b7228b8', x: "3", y: "4", width: "18", height: "18", rx: "2", ry: "2" }), index.h("line", { key: '4df9cc25f1dc1d9cd628e2169b052a44f22ad634', x1: "16", y1: "2", x2: "16", y2: "6" }), index.h("line", { key: '4f42cfac0345263bf425d9be22a3257200ee8a7d', x1: "8", y1: "2", x2: "8", y2: "6" }), index.h("line", { key: 'b9c29ccdc399424b365482715c5e4901c6372aa5', x1: "3", y1: "10", x2: "21", y2: "10" }))), this.renderDatePicker()));
    }
...
async function getMfaStatus(operationsToken, userScopedAccessToken, mfainquiryId, environment = common.Environment.PRODUCTION) {
...
async function resendMfaLink(operationsToken, userScopedAccessToken, requestBody, environment = common.Environment.PRODUCTION) {
    const apiConfig = common.getApiConfig(environment);
...
async function saveCardOnMfaSuccess(operationsToken, userScopedAccessToken, requestBody, environment = common.Environment.PRODUCTION) {
    const apiConfig = common.getApiConfig(environment);
...
async function saveBankOnMfaSuccess(operationsToken, userScopedAccessToken, requestBody, environment = common.Environment.PRODUCTION) {
    const apiConfig = common.getApiConfig(environment);
...
const MfaModelPopUP = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.successEvent = index.createEvent(this, "successEvent");
    }
    operationsToken;
    ...
    async handleMfaResponseChanged(newValue) {
        if (!newValue)
            return;
        if (newValue.mfaStatus === common.MFAStatusEnum.Pass) {
            this.clearPollingTimers();
            let payload;
            let response;
        ...
        else if (newValue.mfaStatus === common.MFAStatusEnum.Failed) {
            this.clearPollingTimers();
    ...
    render() {
        return (index.h("div", { key: '5de043eea173405086f78a6e185e4d80b48ed81f', class: "popup" }, index.h("div", { key: '3c3352e1f220f4ee5aaa7ac171bb2da6ca119bd5', class: "action-modal" }, index.h("div", { key: '895772d846b9e6a5f5739ce998631e1a60cbeee2', class: "modal-container" }, index.h("div", { key: 'a4ab42afb23c614bec19b23b41d52200034e8241', class: "modal-header" }, index.h("h3", { key: '22e77c17a562b26cab4dc05e947e4f02ba37cf5d' }, "Verification Link Sent")), index.h("div", { key: '66b16cbbf996f68c603413aaf16a8cfdab814469', class: "modal-body" }, index.h("p", { key: '9e26756546f5a85bb50d9d2a04877b78774611ca', class: "dialogContentText" }, "A verification link has been sent to your ", index.h("span", { key: 'beb3ef90f640e49857fad56d1935e3c4cc95a0e7', class: "highlight" }, "mobile number ending in ", this.mobileNumberLastFourDigits, "."), index.h("br", { key: 'bbd0feb709fc2295df6c2952e29d1c1359c64809' }), "Please tap the link and follow the instructions to confirm your identity.", index.h("br", { key: '66c011c5c5d70fc71ada769fa65c7a407ace6a39' }), index.h("br", { key: 'e69b20f14910a18c8036e0f13a64a612c4106ada' }), "The link will expire in ", index.h("span", { key: 'b7f974b1b09a05ba2e54c6bba8c1b6ea691edc45', class: "time-highlight" }, this.formatTime(this.time)), ".", index.h("br", { key: 'a86388a63e57e3f807f79e6ce092df3846d2ff25' }), "You have ", this.noOfAttempts, " remaining ", this.getAttemptLabel(), " to resend the link if needed."), index.h("div", { key: 'e4453569af76aa3d57958810180a9a413ee81642', class: "modal-footer" }, index.h("div", { key: '57947d11ec1f9479a87081c596442a3674bcfd19', class: "button-box" }, index.h("button", { key: '213402979888e5227f5b9adec8133bc26178a6c2', class: "secondarybtn", onClick: this.handleCancel }, "Cancel"), index.h("button", { key: '543da21f48b02a8f383b5639815b905f9941405c', class: "primary-btn", disabled: this.isRunning, onClick: this.handleResendLinkClick }, "Resend Link"))))))));
    }
    static get watchers() { return {
...
exports.mfa_model_popup = MfaModelPopUP;
//# sourceMappingURL=date-picker.mfa-model-popup.entry.cjs.js.map

//# sourceMappingURL=date-picker_2.cjs.entry.js.map